...
public class PlayerController implements Initializable {
    ...
    private void handleReady() {
        ...
        double durationdisplay = duration.toMillis();
        lengthText.setText(String.format("%.1f", durationdisplay));
        
       Duration ct = mediaPlayer.getCurrentTime();
       
        double ctDisplay = ct.toMillis();
        
        currentText.setText(String.format("%.1f", ctDisplay));
        currentVisualizer.start(numBands, vizPane);
        timeSlider.setMin(0);
        timeSlider.setMax(duration.toMillis());
        this.timeSlider.valueChangingProperty().addListener((ObservableValue< ?extends Boolean >observable,Boolean oldValue, final Boolean newValue)->{
            mediaPlayer.pause();
            if(!newValue){
    ...
    @FXML
    private void dragstick(ActionEvent event) {
